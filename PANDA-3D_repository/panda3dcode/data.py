...
class Alphabet(object):
    def __init__(
        ...
        prepend_toks: Sequence[str] = ("<go>", "<pad>", "<eos>", "<unk>"),
    ...
    def from_architecture(cls, name: str) -> "Alphabet":
        if name in ("ESM-1", "protein_bert_base"):
            ...
            prepend_toks: Tuple[str, ...] = ("<go>", "<pad>", "<eos>", "<unk>")
...
class Alphabet_goclean(Alphabet):
    def __init__(
        self,
        ...
        self.all_toks.extend(self.standard_toks)
        for i in range((8 - (len(self.all_toks) % 8)) % 8):
            self.all_toks.append(f"<null_{i  + 0}>")
        self.all_toks.extend(self.append_toks)
        ...
        self.unique_no_split_tokens = self.all_toks
    def tolabel(self, gos, **kwargs):
        """
        input: list of go: str
        return: label: tensor
        """
        label_ = torch.zeros((1,len(self.all_toks)), dtype=torch.float32)
        for go in gos:
            if go in self.tok_to_idx: # only tokenize GO in dictionary
                label_[0,self.tok_to_idx[go]] = 1.0
        return label_
