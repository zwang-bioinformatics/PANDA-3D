...
class GVPTransformerEncoder(nn.Module):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.embed_gvp_input_features = nn.Linear(15, args.gvp_node_input_dim_scalar)
        self.embed_confidence = nn.Linear(16, args.gvp_node_input_dim_scalar)
        self.embed_dihedrals = DihedralFeatures(args.gvp_node_input_dim_scalar)
        ...
        self.embed_gvp_output = nn.Linear(gvp_out_dim, args.gvp_node_input_dim_scalar)
        ...
        self.esm_gvp = nn.Linear(args.esm_dim, args.gvp_node_input_dim_scalar)
        nn.init.normal_(self.esm_gvp.weight, mean=0, std=args.esm_dim ** -0.5)
        #self.layer_norm = nn.LayerNorm(embed_dim)

    def build_encoder_layer(self, args):
        return TransformerEncoderLayer(args)

    def forward_embedding(self, esms, coords, seqs, padding_mask, confidence):
        """
        Args:
        ...
        #)
        components["tokens"] = self.embed_tokens(seqs) * self.embed_scale
        ...
        gvp_encoder:
        """
        gvp_out_scalars, gvp_out_vectors = self.gvp_encoder(esms, coords,
    ...
    def forward(
        ...
        """
        esms_gvp_input = self.esm_gvp(esms)
        esms_gvp_output, encoder_embedding = self.forward_embedding(esms_gvp_input, coords, seqs,
                encoder_padding_mask, confidence)
        #x = torch.cat(esms_gvp_input, esms_gvp_output)
        x = esms_gvp_input + esms_gvp_output
        #print('gvp_transformer_encoder 84 esms', x.shape)
